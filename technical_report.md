...
- `include/history.c` - maintain history related functions
...
```
User Input → Tokenizer → Parser → Command Structure → Executor → Display on Terminal
```
...
- If anything is not implemented it will THROW ans error
...
4. Identifies pipeline operator `|` (Single pipe)
...
### 2.3.2 Input/Output Redirection Handling
...
### 2.4.1 Steps for Pipeline Execution
...
### 3.2 SIGCHLD — Zombie Process Cleanup
...
| Zombie Process Cleanup | Supported |
|Many more ...|
...
-` Chirag Suthar`
![Alt text](/include/Chirag.png)

-`RajKumar`
![Alt text](/include/Raj.png)
